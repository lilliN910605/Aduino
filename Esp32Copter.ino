...
#define PWMOUT  // 正常 esc，取消註釋串行 esc
#define LED 2
#define CALSTEPS 256 // 陀螺儀和加速度計校準步驟
//#define externRC // 在 ppmsum 模式下使用外部 RC 接收器
//#define webServer // 使用網絡服務器更改 PID

extern int16_t accZero[3];
extern float yawRate; //yawRate-5.0 from pid.c
...
//volatile這變數為一個硬體硬體的站存器 會被硬體所改變 有時一圈變數的值改變了 compiler 並不會馬上將他寫路記憶體中
//會先打結果放在CPU站存器中 等到處理結束之後 才會寫入記憶體
...
void loop() 
{
  ...
  if (recv)
  {
    ...
    {  
      if (rcValue[THR] < MINTHROTTLE) armct++; //armct=0 MINTHROTTLE=1090
      if (armct >= 25) 
      { 
